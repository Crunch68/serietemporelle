...
#test de Mann-Kendall
  mk.test(trafic_routier,alternative=c("greater")) #rejet de H0, la série a une tendance haussière

  
#les tests seront donc effectués sur des modèles avec tendance et constante
  library(urca)
  
#test ADF
  #sélection du lag optimal
    #basé sur la longueur de la série et venant de la fonction adf.test
      k=trunc(length((trafic_routier)-1)^(1/3))
      adf<-ur.df(trafic_routier, type=c("trend"), lags=k, selectlags=c("Fixed"))
      summary(adf)
      plot(adf) #les résidus ne sont pas bruit blanc, le test n'est pas significatif
    #basé sur les critères AIC, MAIC, BIC, HQC (convergent vers un lag=13)
      T<-length(trafic_routier)
      pmax<-as.integer(12*(T/100)^(0.25))
      install.packages("CADFtest")
      library(CADFtest)
      r<-CADFtest(trafic_routier,criterion="MAIC",type="trend",max.lag.y=pmax)
      plot(r) #les résidus ne sont pas bruit blanc, le test n'est pas significatif
    
#test de Phillips-Perron
  pptest<-ur.pp(trafic_routier, type="Z-tau", model="trend", lags="short")
  summary(pptest) #la série ne possède pas de racine unitaire (-7.1286 < -3.427043) pour le modèle avec tendance
  plot(pptest) #les résidus ne sont pas bruit blanc, le test n'est pas significatif

  pptest<-ur.pp(trafic_routier, type="Z-tau", model="constant", lags="short")
  summary(pptest) #la série ne possède pas de racine unitaire (-6.7394 < -2.871844) pour le modèle avec constante
  plot(pptest) #les résidus ne sont pas bruit blanc, le test n'est pas significatif

#test KPSS
  testKPSStau<-ur.kpss(trafic_routier, type="tau")
  summary(testKPSStau) #la série possède au moins une racine unitaire (0.171>0146)
  plot(testKPSStau) #les résidus ne sont pas bruit blanc, le test n'est pas significatif

  testKPSSmu<-ur.kpss(trafic_routier, type="mu")
  summary(testKPSSmu) #la série possède au moins une racine unitaire (1.2777>0.463)
  plot(testKPSSmu) #les résidus ne sont pas bruit blanc, le test n'est pas significatif

#aucun test n'étant significatif, on peut soupçonner (et également par lecture graphique) la présence d'un changement structurel
  
#test de Zivot-Andrews
  zatest<-ur.za(trafic_routier,model=c("both"),lag=13)
  summary(zatest) #rejet de H0, la série est stationnaire avec un changement structurel (Covid) 
  plot(zatest)
...
#test de Lee Strazicich
  ...
  #réalisation du test
    ur.ls(trafic_routier, model=c("break"), breaks=1, lags=NULL, method=c("GTOS"), pn=0.1, print.results="print") #stationnaire

#La série est donc I(0)+TS, il faut donc la dé-trender
  dummy_trend<-1:length(trafic_routier)
  lm1<-lm(trafic_routier~dummy_trend)
  plot.ts(lm1$residuals)
  abline(h=0, col="red")

  summary(lm1$residuals) #moyenne centrée en 0
  cs.test(lm1$residuals) #la trend a bien été retirée
  mk.test(lm1$residuals) #la trend a bien été retirée

#Vérification de la stationarité de la série après avoir retirée la trend
  adf.test(lm1$residuals)
  pp.test(lm1$residuals)
  kpss.test(lm1$residuals)
  za<-ur.za(lm1$residuals, model = c("intercept"), lag=12)
  summary(za)
  plot(za)
  #la série est bien I(0)